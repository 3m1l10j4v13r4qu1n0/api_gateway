...
Este repositorio (`api_gateway`) es un **API Gateway** FastAPI (proxy reverso): no tiene
dominio ni casos de uso. Antes de diseñar endpoints propios del gateway (dashboard,
health), leer `references/fastapi-conventions.md` (naming, typing vigente) y usar
`templates/endpoint_fastapi.py` como scaffold. El resto de endpoints expuestos NO se diseñan
acá: son de los microservicios (`/auth/*`, `/usuarios/*`, `/roles/*` → :8001;
`/afiliados/*`, `/sync/*` → :8002) y la gateway solo los reenvía según la tabla de
`app/core/routing.py`. Seguir `.agents/rules/reglas-solid.md` (adaptada a gateway) y, para
agregar rutas/servicios, el skill `proxy-gateway` (flujo de 6 pasos).
